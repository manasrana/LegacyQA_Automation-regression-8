...
public class OrderReviewAndConfirmPage {
    ...
    @FindBy(xpath = ControlLocators.DUE_AMOUNT_FOR_DEVICE1)
    public WebElement dueAmountForDevice1;

    @FindBy(xpath = ControlLocators.DUE_AMOUNT_FOR_DEVICE2)
    public WebElement dueAmountForDevice2;

    @FindBy(xpath = ControlLocators.TOTAL_DUE_MONTHLY_VALUE)
    public WebElement totalDueMonthly;
    ...
    public boolean OrderReviewConfirmPageAssertionsFor2Devices() {
        try {
            // Assert.assertEquals(device1PriceActual.getText(), device1Price);
            // Assert.assertEquals(device2PriceActual.getText(), device2Price);
            String monthlyFee = totalFeeActual.getText();
