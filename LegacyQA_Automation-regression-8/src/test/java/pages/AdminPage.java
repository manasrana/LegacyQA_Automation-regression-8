...
public class AdminPage {
    ...
    @FindBy(linkText = ControlLocators.RSG_TIMEOUT_CONFIGURATOR)
    public WebElement rsgTimeoutConfigurator;

    @FindBy(xpath = ControlLocators.WIDGET_ACTIVE_IN_BRAND)
    public WebElement widgetActiveInBrand;

    @FindBy(linkText = ControlLocators.EDIT_WIDGET)
    public WebElement editWidget;

    @FindBy(linkText = ControlLocators.EDIT_BRAND)
    public WebElement editBrand;

    @FindBy(xpath = ControlLocators.MODIFY_RECORD)
    public WebElement modifyRecord;
...
//        }
        ...
        if (beforeRunningTestCount < errorList.size()) {
            for (int start = beforeRunningTestCount; start <= errorList.size() - 1; start++) {
                if (errorList.get(start).toLowerCase().contains(new String(IGNORED_DB_ERROR).toLowerCase())) {
                    Reporter.log("Error:" + errorList.get(start));
                    isPass = true;
    ...
    public void widgetManagement_QA240() throws IOException {
        BrowserSettings.driver.navigate().to(
                BrowserSettings.readConfig("baseUrl")
                        + ControlLocators.WIDGET_MANAGEMENT);
        Utilities.implicitWaitSleep(3000);
        widgetNameSearchTextbox.sendKeys("RSG Timeout Configurator");
        widgetSearchButton.click();
        Utilities.waitForElementVisible(rsgTimeoutConfigurator);
        editBrand.click();
        //Utilities.waitForElementVisible(widgetDefaultActive);
        editWidget.click();

        if (widgetActiveInBrand.isSelected()) {
            widgetActiveInBrand.click();
        }
    ...
    public void navigateToGlobalAlert() throws IOException {
        BrowserSettings.driver.navigate().to(
                BrowserSettings.readConfig("baseUrl")
                        + "/admin/globaladvisory/listglobalalerts.php/");
    }
}