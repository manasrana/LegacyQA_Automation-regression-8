...
//endregion QA-52
    ...
    @Test(groups = {"Misc"})
    @Parameters("test-type")
    public void QA_240AddLineForVerizonWhenSystemIsDown(@Optional String testtype) {
        try {
            IMEIDetails imeiDetails1 = null;
            IMEIDetails imeiDetails2 = null;
            String carrierType = "Verizon";
            ...
            Reporter.log("<h2>Start - QA_240AddLineForVerizonWhenSystemIsDown. <br></h2>");
            Reporter.log("<h3>Description: Configurable RSG-Configurable Duration of Failed Credit Check and Activation " +
                    "   Place a Add a line order for Verizon when System is down before Activation fail timeout.</h3>");
            Reporter.log("Launching Browser <br>", true);
            Log.startTestCase("QA_240AddLineForVerizonWhenSystemIsDown");
            ...
            AdminBaseClass adminBaseClass = new AdminBaseClass();
            adminBaseClass.launchAdminInNewTab();
            PageBase.AdminPage().widgetManagement_QA240();

            //Switching Back To Retail
            Utilities.switchPreviousTab();

            //POA FLOW
            orderId = poaFlowQA240(testtype, imeiDetails1.IMEI, imeiDetails1.Sim, orderId, receiptId, phoneNumber,
                    sSN, zipCode, accountPassword);
            ...
            Reporter.log("<h2>End - QA_240AddLineForVerizonWhenSystemIsDown test case completes. <br></h2>");
            Log.endTestCase("QA_240AddLineForVerizonWhenSystemIsDown");

        } catch (Exception ex) {
            Log.error(ex.getMessage());
            System.out.println(ex.getMessage());
            Utilities.driverTakesScreenshot("QA_240AddLineForVerizonWhenSystemIsDown");
            Assert.assertTrue(false);
        }
    }
    ...
    @Test(groups = {"Misc"})
    @Parameters("test-type")
    public void QA_5995_EditGlobalAdvisoryPage(@Optional String testType) throws InterruptedException, AWTException, IOException {

        Log.startTestCase("QA_5995_EditGlobalAdvisoryPage");
        Reporter.log("<h2>QA_5995_EditGlobalAdvisoryPage. <br></h2>");
        Reporter.log("Launching Browser <br>", true);
        ...
        PageBase.AdminPage().navigateToGlobalAlert();


        Reporter.log("<h3>QA_5995_EditGlobalAdvisoryPage - Test Case Completes<h3>");
        Log.endTestCase("QA_5995_EditGlobalAdvisoryPage");
...
//endregion QA-52 Refactored Methods
    ...
    //region QA 240 refactored methods
    private String poaFlowQA240(@Optional String testtype, String iMEINumber1, String simNumber1,
                                String orderId, String receiptId, String phoneNumber, String sSN, String zipCode, String accountPassword)
            throws IOException, AWTException, InterruptedException {
...
//                portDetails.CurrentAccNumber, portDetails.SSN);
        ...
        if (readConfig("Activation").toLowerCase().contains("true")) {
            ...
            //Device Verification and Activation page. Scan Device IEMI and enter SIM number.
            Utilities.waitForElementVisible(PageBase.DeviceVerificationaandActivation().deviceIMEITextbox);
            PageBase.DeviceVerificationaandActivation().deviceVerificationActiavtionFor1Device(iMEINumber1, simNumber1);
...
//            catch (Exception e) {}
            ...
            // Set flags to make Verizon System down
            setFlagsQA_240(testtype);
            ...
            QA_240ShipAdminVerification(orderId);
    ...
    // Set flags to make Verizon System Down
    private void setFlagsQA_240(String testType) throws InterruptedException, AWTException, IOException {
        if (testType.equals("internal")) {
            ...
            driver.findElement(By.id("down[1008]")).click();
    ...
    private void QA_240ShipAdminVerification(String orderId) throws IOException {
        //Ship Admin
        ShipAdminBaseClass.launchShipAdminInNewTab();
        PageBase.OrderSummaryPage().goToOrderSummaryPage(orderId);
        String eventLogTableContent = PageBase.OrderSummaryPage().checkForErrorAndLog(orderId);
        String status = PageBase.OrderSummaryPage().getOrderStatus();
        Assert.assertTrue(eventLogTableContent.contains(Constants.ORDER_VALIDATION_FAILED));
    }

    //endregion Q 240 refactored methods

    //region QA 5995 refactored Methods

    //endregion QA 5995 refactored methods
